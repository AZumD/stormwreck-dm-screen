(function () {
  ...
  const STORAGE_KEY = "stormwreck-character-creator-v2";
  ...
  function uses2014BonusModel() { return state.ruleset === "2014" || (state.ruleset === "both" && state.bonusSource === "2014"); }
  function uses2024BonusModel() { return state.ruleset === "2024" || (state.ruleset === "both" && state.bonusSource === "2024"); }
  ...
  function applyFlexibleBoost(out, pool, selection) {
    ...
    const secondary = selection.secondary && pool.includes(selection.secondary) && selection.secondary !== primary
      ? selection.secondary
      : secondaryDefault;
  ...
  function abilityBonuses() {
    ...
    if (uses2014BonusModel()) {
      ...
      if (species?.flexible2014) {
        applyFlexibleBoost(out, ABILITIES, state.speciesBonuses);
      } else {
    ...
    applyFlexibleBoost(out, backgroundAbilityPool(bg), state.backgroundBonuses);
    return out;
  }
  ...
  function renderClass() {
    const list = CLASSES.filter((x) => visibleByRules(x) && (state.classComplexity === "all" || x.complexity.toLowerCase() === state.classComplexity));
    return head("Step 2", "Choose a class", "Core classes live beside expanded official options. The complexity filter is a convenience, not a rule.") +
      ...
      `<div class="option-grid">${list.map((c) =>
        ...
        `<span class="option-card__tags"><span class="mini-tag">${esc(c.complexity)}</span>${c.caster ? `<span class="mini-tag">Spellcasting</span>` : ""}${sourceTag(c)}${tagRules(c)}</span>` +
        `</button>`
      ).join("")}</div>`;
  }
  ...
  function renderSpecies() {
    ...
    return head("Step 3", "Choose a species", modeText) +
      `<div class="option-grid">${items.map((s) => {
        ...
        return `<button type="button" class="option-card${selected(state.species, s.name)}" data-species="${esc(s.name)}">` +
          ...
          `<span class="option-card__tags">${sourceTag(s)}${tagRules(s)}</span></button>`;
  ...
  function renderBackground() {
    ...
    const bothToggle = state.ruleset === "both"
      ...
      : "";
    return head("Step 4", "Choose a background", "Background describes the life the character had before adventuring. Expanded legacy backgrounds are converted when used with the 2024 rules.") + bothToggle +
  ...
  function renderAbilities() {
    ...
    if (state.abilityMethod === "roll") {
      status = `<div class="roll-bank">${(state.rolls.length ? state.rolls : ["Roll to generate six scores"]).map((x) => `<span class="roll-chip">${esc(x)}</span>`).join("")}` +
        `<button type="button" class="creator-btn creator-btn--ghost" data-roll-abilities>Roll 4d6 × 6</button></div>`;
    }
  ...
  function abilityCard(k, bonus, finalScore) {
    ...
    if (state.abilityMethod === "standard") {
      control = `<select data-ability="${k}">${STANDARD_ARRAY.map((n) => `<option value="${n}"${Number(state.abilities[k]) === n ? " selected" : ""}>${n}</option>`).join("")}</select>`;
    } else if (state.abilityMethod === "pointbuy") {
      control = `<select data-ability="${k}">${[8, 9, 10, 11, 12, 13, 14, 15].map((n) => `<option value="${n}"${Number(state.abilities[k]) === n ? " selected" : ""}>${n} · ${POINT_COST[n]} pts</option>`).join("")}</select>`;
    } else {
      control = `<input data-ability="${k}" type="number" min="3" max="20" value="${esc(state.abilities[k])}">`;
    }
    return `<article class="ability-card" data-ability="${k}"><div class="ability-card__head"><span class="ability-card__abbr">${k.toUpperCase()}</span>` +
      `<span class="ability-card__mod">${modText(finalScore)}</span></div><div class="ability-card__score">${finalScore}</div>${control}` +
      `<div class="ability-card__bonus">Base ${state.abilities[k]}${bonus ? ` · rules bonus +${bonus}` : " · no rules bonus"}</div></article>`;
  }

  function renderFeats() {
    const bg = currentBackground();
    const requiresOrigin = uses2024BonusModel();
    const suggested = requiresOrigin ? bg?.feat : "";
    return head("Step 6", "Feats", requiresOrigin
      ? "2024 backgrounds grant an Origin feat. Older backgrounds get to choose one during conversion."
      ...
      `<span class="option-card__title">No feat / decide later</span></button>${FEATS.map((f) =>
        `<button type="button" class="option-card${selected(state.feat || suggested, f)}" data-feat="${esc(f)}"><span class="option-card__title">${esc(f)}</span>` +
        `<span class="option-card__meta">Origin feat option</span></button>`
      ).join("")}</div>`;
  }
  ...
  function renderSpells() {
    ...
    return head("Step 7", "Spells", "Pick a starter shortlist. The pool is intentionally broad; class-specific legality can be refined against the compendium later.") +
      `<div class="option-grid">${SPELLS.map((s) => `<button type="button" class="option-card${state.spells.includes(s) ? " is-selected" : ""}" data-spell="${esc(s)}">` +
      `<span class="option-card__title">${esc(s)}</span><span class="option-card__meta">${state.spells.includes(s) ? "Selected" : "Add to shortlist"}</span></button>`).join("")}</div>`;
  }
  ...
  function renderEquipment() {
    return head("Step 8", "Equipment", "Choose a starting-equipment approach. Detailed inventory can still be adjusted from the Player Companion after creation.") +
      `<div class="option-grid">${EQUIPMENT.map((e) => `<button type="button" class="option-card${selected(state.equipment, e)}" data-equipment="${esc(e)}">` +
      `<span class="option-card__title">${esc(e)}</span><span class="option-card__meta">${e === "Custom / decide later" ? "Leave the detailed loadout open." : "Use this as the character's starting equipment note."}</span></button>`).join("")}</div>`;
  }
  ...
  function renderReview() {
    const finals = finalAbilities();
    const bg = currentBackground();
    const feat = state.feat || (uses2024BonusModel() ? bg?.feat || "" : "");
    ...
    return head("Step 11", "Review", "One last pass before this pile of choices becomes an actual character.") +
      ...
      `<p class="creator-lede" style="margin:.35rem 0 0">Saving creates this character directly in your signed-in Player Companion account.</p>` +
  ...
  function exportPayload() {
    const bg = currentBackground();
    const originFeat = state.feat || (uses2024BonusModel() ? bg?.feat || "" : "");
    return {
      format: "stormwreck-character-creator-v2",
  ...
  async function saveCharacter() {
    ...
    try {
      ...
      setState({
        ...
        saveMessage: `${character.name} was created in the Player Companion.`,
        saveError: ""
      });
    } catch (err) {
  ...
  els.content.addEventListener("click", (e) => {
    ...
    if (t.dataset.ruleset) {
      ...
      if (state.className && !CLASSES.find((x) => x.name === state.className)?.rules.includes(ruleset) && ruleset !== "both") patch.className = "";
    ...
    if (t.dataset.class) {
      ...
      setState({ className: t.dataset.class, spells: chosen?.caster ? state.spells : [] });
      return;
    }
    ...
    if (t.dataset.spell) {
      ...
      const spells = state.spells.includes(spell) ? state.spells.filter((x) => x !== spell) : [...state.spells, spell];
      setState({ spells });
      return;
    }
